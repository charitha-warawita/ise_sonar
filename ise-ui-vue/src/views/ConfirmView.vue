<template>
	...
	<div>
		<div class="row">
			<div class="col-md-6">
				<h5>ISE spec - Request</h5>
				<hr />
				<pre>{{ project }}</pre>
			</div>
			<div class="col-md-6">
				<h5>Converted Cint Specs - Requests</h5>
				<hr />
				<div v-for="request in cintRequests" :key="request.name">
					<label
						><b>Cint Conversion validation result: </b
						>{{ request.validationResult }}</label
					>
					<hr />
					<label
						><b>Cint Request:</b> {{ request.request.name }}</label
					>
					<pre>{{ request }}</pre>
					<hr />
				</div>
...
async function createNewProject() {
	...
	if (
		typeof useProjStore.project.id === undefined ||
		useProjStore.project.errors.length > 0
	) {
		var newLine = "\r\n";
		var alertMessage =
			"Project Save as Draft is unsuccessful. Project ID returned is: " +
			useProjStore.project.id +
			"." +
			newLine;
		...
		alert(
			"project successfully created. New project ID is " +
				useProjStore.project.id
		);
		useProjStore.$reset();
		router.push("/");
...
function discardNewProject() {
	if (
		confirm(
			"Are you sure you want to cancel? All your changes will be discarded?"
		)
	) {
		useProjStore.$reset();
		router.push("/");
