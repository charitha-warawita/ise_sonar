...
export default {
	...
	async configure(clientId, enableDummyUser = true) {
		...
		if (!clientId && enableDummyUser) {
			console.log(
				"### Azure AD sign-in: disabled. Will run in demo mode with dummy demo@example.net account"
			);
			...
			msalApp = {
				...
				acquireTokenSilent() {
					return new Promise((resolve) =>
						resolve({ accessToken: "1234567890" })
					);
				},
				cacheStorage: {
					clear() {
	...
	async acquireToken(scopes = ["user.read"]) {
		...
		try {
			...
			console.log(
				"### MSAL acquireTokenSilent was successful for scope: " +
					scopes
			);
		} catch (err) {
			...
			console.log(
				"### MSAL acquireTokenPopup was successful for scope: " + scopes
			);
		}
		...
		if (!tokenResp.accessToken) {
			throw new Error(
				"### accessToken not found in response for scopes: " +
					scopes +
					", that's bad"
			);
		}
